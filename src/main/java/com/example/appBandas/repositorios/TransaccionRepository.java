...
public interface TransaccionRepository extends JpaRepository<Transaccion, Integer> {
    ...
    @Query("SELECT COALESCE(SUM(t.cantidad), 0) FROM Transaccion t WHERE t.banda.idBanda = :idBanda AND t.tipo = 'Ingreso' AND t.fecha >= :desde")
    Double sumarIngresosRecientes(Integer idBanda, LocalDate desde);

    @Query("SELECT COALESCE(SUM(t.cantidad), 0) FROM Transaccion t WHERE t.banda.idBanda = :idBanda AND t.tipo = 'Gasto' AND t.fecha >= :desde")
    ...
    Double sumarIngresosPorPeriodo(Integer idBanda, java.time.LocalDate desde);
}